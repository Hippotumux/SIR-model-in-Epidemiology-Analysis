...
public class Human {
	...
	public void init(int x, int y, int infect_rate, int double_infect_rate, int recover_rate, int death_rate, int city, int status) {
		_posX = x;
		_posY = y;
	...
	public void walk(int W, int H, int[][] map, int[][] infect_map) {
		...
		while (_is_in_hole == 0) {
			...
			// 判斷是否在城市裡
			if (map[new_posX][new_posY] == 1 || map[new_posX][new_posY] == 2 || map[new_posX][new_posY] == 3 || map[new_posX][new_posY] == 4 || map[new_posX][new_posY] == 5) {
				_posX = new_posX;
				_posY = new_posY;
				break;
			}
