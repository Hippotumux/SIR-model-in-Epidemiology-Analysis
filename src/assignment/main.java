...
public class main extends Application {
	...
	public void create_city_and_people(Parameter para, int[][] map) {
		...
		for (int i = 0; i < A_amount; i++) {
			...
			if (i < para.get_city1_initial_infections()) {
				...
				new_human.init(rand_x, rand_y, para.get_moving_times(), para.get_reinfection_rate(),
						para.get_recovery_rate(), para.get_mortality_rate(), 1, 1);
				...
				Human new_human = new Human();
				new_human.init(rand_x, rand_y, para.get_moving_times(), para.get_reinfection_rate(),
						para.get_recovery_rate(), para.get_mortality_rate(), 1, 0);
				total_people.add(new_human);
			}
		}
		...
		for (int i = 0; i < B_amount; i++) {
			...
			if (i < para.get_city2_initial_infections()) {
				...
				Human new_human = new Human();
				new_human.init(rand_x, rand_y, para.get_moving_times(), para.get_reinfection_rate(),
						para.get_recovery_rate(), para.get_mortality_rate(), 2, 1);
				total_people.add(new_human);
			} else {
				// 沒病
				...
				Human new_human = new Human();
				new_human.init(rand_x, rand_y, para.get_moving_times(), para.get_reinfection_rate(),
						para.get_recovery_rate(), para.get_mortality_rate(), 2, 0);
				total_people.add(new_human);
			}
		}
		...
		for (int i = 0; i < C_amount; i++) {
			...
			if (i < para.get_city3_initial_infections()) {
				...
				Human new_human = new Human();
				new_human.init(rand_x, rand_y, para.get_moving_times(), para.get_reinfection_rate(),
						para.get_recovery_rate(), para.get_mortality_rate(), 3, 1);
				total_people.add(new_human);
			} else {
				// 沒病
				...
				Human new_human = new Human();
				new_human.init(rand_x, rand_y, para.get_moving_times(), para.get_reinfection_rate(),
						para.get_recovery_rate(), para.get_mortality_rate(), 3, 0);
				total_people.add(new_human);
			}
		}
	}
	...
	public void update_map() {
		...
		_total = _health_amount + _infection_amount + _recover_amount;
	}
	...
	public void start(Stage primaryStage) {
		...
		int day_gap = 10;
		int total_day = ui.get_days() * day_gap;
		...
		delay.setOnFinished(subevent -> {
			_total = para.get_total_population();
			DataChart data_chart = new DataChart(ui.get_days(), _total);
			data_chart.start();
			create_city_and_people(para, big_map.getmap());
			init_map();
			update_map();
			data_chart.update_data(0, _recover_amount, _infection_amount, _health_amount, _total);
			final int[] day = { 0 };
			timeline = new Timeline(new KeyFrame(Duration.millis(150), event -> {
				if (day[0] < total_day) {
					...
					if (day[0] != 0 && day[0] % day_gap == 0) {
						...
						data_chart.update_data((int) (day[0] / day_gap), _recover_amount, _infection_amount,
								_health_amount, _total);
					}
					day[0]++;
				} else {
					timeline.stop();
